...
function Get-CanonicalTrackId {
    ...
    $artistKey = @($Artist -split '[,，、/&]' | ForEach-Object { Normalize-MusicText $_ } |
...
function New-CanonicalTrack {
    param(
        ...
        [object]$PreviewSources = $null,
        [object]$DownloadCandidates = $null,
        [string]$LocalSongId = '',
        [ValidateSet('REMOTE','WANTED','RESOLVING','DOWNLOADING','VALIDATING','LOCAL','RETRY_WAIT','UNAVAILABLE')]
...
function Read-StateCollection {
    ...
    try {
        ...
        throw "无法读取状态文件 $path：$($_.Exception.Message)"
    }
}
...
function Save-CanonicalTrack {
    ...
    for ($i = 0; $i -lt $items.Count; $i++) {
        if ([string]$items[$i].id -eq [string]$Track.id) {
            $old = $items[$i]
            if (-not (Get-OptionalProperty $Track 'created_at') -and (Get-OptionalProperty $old 'created_at')) { Set-OptionalProperty -Object $Track -Name 'created_at' -Value $old.created_at }
            $oldStatus = [string](Get-OptionalProperty $old 'status' 'REMOTE')
            if ($oldStatus -ne 'REMOTE' -and [string](Get-OptionalProperty $Track 'status' 'REMOTE') -eq 'REMOTE') { Set-OptionalProperty -Object $Track -Name 'status' -Value $oldStatus }
...
function Save-DailyRecommendations {
    ...
    $history = @(Read-StateCollection -Config $Config -Name recommendation_history |
        Where-Object { [string]$_.date -ne $Date })
    $history += $recs
...
function Import-LegacyRecommendationState {
    ...
    foreach ($row in $rows) {
        $rank++
        $title = [string](Get-OptionalProperty $row 'Title')
        $artist = [string](Get-OptionalProperty $row 'Artist')
        $trackId = [string](Get-OptionalProperty $row 'TrackId')
        if (-not $trackId) { $trackId = Get-CanonicalTrackId -Title $title -Artist $artist }
        $fileName = [string](Get-OptionalProperty $row 'File')
        $localPath = if ($fileName) { Join-Path $Config.DailyDir $fileName } else { '' }
        $isLocal = $localPath -and (Test-Path -LiteralPath $localPath)
        $neteaseId = [string](Get-OptionalProperty $row 'NeteaseId')
        $duration = [int](Get-OptionalProperty $row 'Duration' 0)
        ...
        $trackStatus = if ($isLocal) { 'LOCAL' } else { 'REMOTE' }
        $playbackSource = if ($isLocal) { "navidrome:$trackId" } elseif ($neteaseId) { "netease:$neteaseId" } else { '' }
        ...
        $tracks += $track
        $recs += [pscustomobject]@{
            id = "rec_legacy_${date}_$rank"; date = $date; track_id = $trackId; netease_id = $neteaseId
            title = $title; artist = $artist; duration = $duration; rank = $rank
            reason = [string](Get-OptionalProperty $row 'FromSeed' '历史日推')
            playback_source = $playbackSource
            preview_sources = $preview; liked = [bool]([string](Get-OptionalProperty $row 'Liked' 'False') -eq 'True')
...
function Get-TodayRecommendations {
    ...
    return @(Read-StateCollection -Config $Config -Name recommendations |
...
function Add-WantedTrack {
    ...
    $wanted = [pscustomobject]@{
        id = "wanted_$([guid]::NewGuid().ToString('N'))"
        track_id = $TrackId
        state = 'WANTED'
        attempts = 0
...
function Save-WantedTrack {
    ...
    $items = @(Read-StateCollection -Config $Config -Name wanted)
    $Wanted.updated_at = Get-NowIso
    $found = $false
    for ($i = 0; $i -lt $items.Count; $i++) {
        if ([string]$items[$i].id -eq [string]$Wanted.id) { $items[$i] = $Wanted; $found = $true; break }
    }
...
function Set-TrackStatus {
    ...
    $track.status = $Status
    if ($LocalSongId) { $track.local_song_id = $LocalSongId }
...
function Set-TrackLike {
    param(
        [Parameter(Mandatory)][psobject]$Config,
        [Parameter(Mandatory)][string]$TrackId,
        [Parameter(Mandatory)][bool]$Liked
    )
    foreach ($stateName in @('recommendations','recommendation_history')) {
        $recs = @(Read-StateCollection -Config $Config -Name $stateName)
        $changed = $false
        foreach ($rec in $recs) {
            if ([string]$rec.track_id -eq $TrackId) {
                Set-OptionalProperty -Object $rec -Name 'liked' -Value $Liked
                Set-OptionalProperty -Object $rec -Name 'updated_at' -Value (Get-NowIso)
                $changed = $true
            }
        }
...
function Write-StructuredEvent {
    ...
    Initialize-MusicServerState -Config $Config
    $row = [ordered]@{
        timestamp = Get-NowIso; track_id = $TrackId; provider = $Provider; event = $Event
        from_state = $FromState; to_state = $ToState; attempt = $Attempt
        duration_ms = $DurationMs; result = $Result; error_type = $ErrorType
...
function Find-LocalTrack {
    ...
    $titleKey = Normalize-MusicText $Title
    if (-not $titleKey) { return $null }
    $artistKey = Normalize-MusicText $Artist
    $exact = Normalize-MusicText "$Title - $Artist"
    $requiresStrictShortTitleMatch = $titleKey.Length -le 1
