...
- First use is owned by `MusicServer.Onboarding.psm1` and `web/onboarding.js`. Guide preferences, completion and selected track live in SQLite `app_settings`. `POST /api/onboarding` prepares fourteen metadata-only starter tracks without network/download work; `Save-DailyRecommendationsDb -OnlyIfEmpty` checks inside the transaction so it cannot overwrite a concurrent personalized day. `onboarding_starter` never marks personalized generation complete; empty provider output preserves an existing day. Existing listening/like history suppresses unsolicited onboarding, and existing display preferences remain authoritative.
- Local lyrics use exact adjacent names, then same-directory `Lyrics/` and `歌词/`. Automatic matches live in SQLite `local_lyric_cache`, keyed by path/fingerprint with a 60-second owner lease; READY works offline, uncertain/provider failures cool down 30 minutes, missing identity/lyrics seven days. Matching requires a reliable artist and existing provider identity checks. The isolated media job binds SQLite before lookup; audio/control requests do not wait for it. `lyrics_url` is always addressable, while `has_local_lyrics` controls the offline-lyrics filter. State/API coverage is `Onboarding` / `OnboardingRuntime`; include the Onboarding module in runtime manifests and API fixtures.

- Music-tree navigation uses one continuous library-coordinate curve (`treeGeometry`) for the trunk and all seven leaf joints. Scrolling interpolates its position; do not restore a separate trunk rotation or fixed leaf anchors. Recommendation focus follows changes to the shared player's `currentKey`, including next/previous and natural autoplay, while pause/poll updates preserve deliberate browsing. Group navigation and the round 56 px play button share the full window's horizontal midpoint, independently of the unequal library/recommendation columns. Desktop UI checks can attach Playwright CLI to an isolated Tauri WebView using process-local `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port=<test-port>` and `WEBVIEW2_USER_DATA_FOLDER`; never enable debugging in production defaults. Close the test APP and its owned services after verification.
...
- Native minimize keeps the taskbar entry while the tray icon remains available. Do not call `window.hide()` on minimize. Tray left-click restores with `unminimize` + `show` + `set_focus`. A missing tray icon does not affect taskbar recovery. Close X still exits and `WindowEvent::Destroyed` stops only APP-owned services. `tests/MusicServer.Tauri.Tests.ps1` pins this contract; actual taskbar/tray interaction requires desktop validation.
...
## 1.0 desktop care checkpoint — 2026-09-14

- `MusicServer.Search.psm1` owns explicit online discovery (`POST /api/search`, `GET /api/search/{id}`); include it in staging, runtime identity/manifest, build watch lists and fixtures. SQLite `online_searches` / `online_search_results` own results and expiry. NetEase requests run in at most two background runspaces, use the shared provider circuit and a 7-second HTTP timeout / 20-second job deadline. Search never modifies the daily mix or enqueues downloads; an explicit like uses the existing atomic transaction and worker. Exact provider IDs reuse canonical tracks without overwriting local bindings; distinct recordings must not collapse on title/artist normalization. SearchRuntime tests exercise slow-provider responsiveness through real PS5.1 API/proxy services.
- Download finalization persists a stable local path identity in the same transaction as LOCAL, including on installed desktops without Navidrome. Optional index lookup must not overwrite it with an empty ID. Older LOCAL records without a binding resolve only through the worker's exact SQLite `recommendation_files` association and an existing file, never title guessing.
- `web/pond-water.js` is a served runtime asset. Water rendering uses cached opaque sky imagery, bounded resolution/frame rate, hidden-page suspension and reduced-motion support; transparent cloud gradients retain their RGB at zero alpha to avoid gray fringes. The tree's six decorative sprigs follow the same scrolling curve geometry as song leaves.

- `MusicServer.Management.psm1` and `manage_musicserver.ps1` own component setup, health probes, backups and redacted diagnostics. Include both in Identity, staging, Rust runtime manifest/build watch lists and hermetic runtime fixtures. `web/management.js` must be served by the launcher's explicit static routes.
- SQLite `maintenance_jobs` is the sole task authority, one RUNNING task at a time. The supervisor bounds a job to 600 seconds and kills its tree on timeout. Components are pinned by exact release, size and SHA-256; verify executable versions and actual audio conversion before atomic directory activation. Explicit executable environment overrides win, then the verified managed directory, then PATH. Workers rediscover tools each pass and do not burn attempts while dependencies are absent.
- Backups use SQLite `.backup`. Restore is an APP command requiring owned services, stops that tree, validates ID/hash/schema/integrity and saves a safety snapshot. `.backup` does NOT checkpoint the source: require successful WAL checkpoint before replacing the database, so a failed replacement preserves old commits. Music and credentials are excluded from backup and never moved by restore. Diagnostic exports use selected fields, never raw logs or DBs.
- Known NetEase/Bilibili sources are tried before search; the worker allows one explicit search fallback. Search uses bounded process exit status rather than ambient LASTEXITCODE. Duration cannot substitute for missing singer evidence. Downloads stage independently and require full audio decoding before publication; retry/next UI uses a 15-second playback deadline and guards against stale selections.
- Incomplete downloads live under APP_HOME `download-staging`, outside the scanned music tree. Cross-volume publication copies to an unindexed `.part` file in MusicDir, then renames in that directory; an existing audio or adjacent lyric gets a unique new name. The isolated media runspace must include `Get-LibraryFolderArtist` so a cold unindexed local stream can resolve before GET /api/library warms the cache.
- APP-owned services are reclaimed on both main-window destruction and `RunEvent::Exit`. Closing during setup can bypass the window callback, and Tauri exits without dropping managed state. Desktop smoke must use a normal window-close request and include close-during-setup; forced `taskkill /T` is failure cleanup only and cannot prove the APP's shutdown behavior.
