...
- Recommendation artwork, click area and wave clipping share the liquid-garden outline. Botanical layers and floating leaves decorate the transparent surface; text and hit targets stay still. Keep all seven songs usable at small window sizes. Playback waves continue while browsing another recommendation group, with smooth origin movement and bounded, fading crests.
- Rain, drifting leaves, ripples and vines have bounded particle/frame budgets; stop animation on hidden/reduced-motion pages. Leaves share the pond clock and recycle only while transparent. Spectrum reads a captured audio copy and never reroutes playback. Recommendation waves use real audio energy where capture is allowed; uncapturable remote streams have quiet playback-progress ripples, never synthetic frequency bars. Pause clears waves; buffering does not advance them.
...
- Desktop startup first checks the three legacy UI/API pairs, then tries at most two OS-selected vacant pairs. Reuse requires the current build AND a SHA-256 scope of the normalized APP-home path; another home's service is never reused. Unknown listeners remain untouched. Native IPC is granted only to the verified UI origin at runtime, including its chosen port, never to a localhost wildcard.
- The embedded `desktop-start.html` has no data polling or artificial delay. Service preparation runs off the UI thread so the opening page can render; closing marks startup cancelled before collecting owned children, preventing a late spawn. Only verified services receive navigation to the product workspace; failures navigate to a durable retry page. Rust writes a bounded latest report to `APP_HOME/logs/desktop-startup.json` with process, selected ports and reason. A rendered main window is not proof that its backend started.
