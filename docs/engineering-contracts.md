...
- Group navigation and the circular player control align with the whole window midpoint, not unequal columns. Keep labels readable through hover/active states. Decorative art never intercepts clicks or obscures song controls.
- Rain, ripples and vines have bounded particle/frame budgets; stop animation on hidden/reduced-motion pages. Canvas gradients keep their RGB at zero alpha to avoid gray fringes. Spectrum reads a captured audio copy and never reroutes playback; unsupported capture stays quiet.
