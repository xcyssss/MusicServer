...
const { LeafWindow, orbitItems } = require('../web/music-tree-ui.js');
...
test('recommendation ripples never duplicate the focus, including short and refreshed days', () => {
  ...
  assert.equal(many.orbit.length, 4);
  assert.equal(new Set([many.focus, ...many.orbit].map(x => x.track_id)).size, 5);
