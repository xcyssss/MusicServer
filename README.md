...
1.0.0-rc.2 在首次使用体验的基础上增加网络歌曲搜索：输入歌名或歌手，回车或点击「搜网络」，即可试听和喜欢并下载。音乐树增加随枝干移动的小叶，左右共用澄澈水面、云影与柔和反光。喜欢会进入后台下载队列，完成校验后出现在音乐树中。先看 [普通使用说明](docs/getting-started.zh-CN.md)，验收证据与限制见 [1.0 验收记录](docs/1.0-readiness.zh-CN.md)。
