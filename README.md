...
1.0.0-rc.4 修复 B 站搜索可能被遗留探测状态永久阻挡的问题，区分真正限流、正在探测和恢复冷却。支持网易云与 B 站搜索、喜欢后后台下载。雨中庭院保留细雨、落水涟漪与边缘植物，日推播放新增随音频强弱扩散的水波；无法读取音频的网络试听使用柔和的播放涟漪。音乐树翻页控件在左侧区域居中，下载动态增强文字对比。先看 [普通使用说明](docs/getting-started.zh-CN.md)，验收证据与限制见 [1.0 验收记录](docs/1.0-readiness.zh-CN.md)。
...
0.2.9-rc.1 增加真实音频频谱、树干轻摆、鼠标跟随水光和小水滴向中心的联动。播放采样只读取音频副本，不改变播放器输出；不支持采样的环境保持静态。下载状态和排查说明见 [下载链路与排查](docs/download-reliability.md)。
