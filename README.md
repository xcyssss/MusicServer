...
1.0.0-rc.3 支持网易云和 B 站搜索：输入歌名或歌手，回车或点击「搜网络」，切换来源后即可试听 / 预览视频、喜欢并下载。界面增加持续细雨、落水涟漪和边缘爬山虎，使用有深浅层次的青绿色水面；头像降低透明度并柔化边缘，设置改为横向滑杆图标。喜欢会进入后台下载队列，完成校验后出现在音乐树中。先看 [普通使用说明](docs/getting-started.zh-CN.md)，验收证据与限制见 [1.0 验收记录](docs/1.0-readiness.zh-CN.md)。
