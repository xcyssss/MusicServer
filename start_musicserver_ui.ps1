...
# the task for a packaged APP_HOME and backfill the current day when it has not
# run yet. Failures here must never block the UI/API startup.
function Initialize-MusicServerScheduledTasks {
    ...
    if (-not (Test-Path -LiteralPath $registrar -PathType Leaf)) { return }
    ...
    try {
        ...
        if ($task) {
            ...
            $registered = ($arguments.Length -gt 0) -and ($arguments.IndexOf($generator, [StringComparison]::OrdinalIgnoreCase) -ge 0)
        }
        if (-not $registered) {
            & $registrar -AppHome $Root | Out-Null
            Write-UiLog "Registered scheduled task $taskName"
            $task = Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue
        }
        ...
        if ($info -and $info.LastRunTime) {
            $ranToday = ((([datetime]$info.LastRunTime).Date) -eq ((Get-Date).Date))
        }
        $running = $false
        if ($task) { $running = ([string]$task.State -eq 'Running') }
        if ((-not $running) -and (-not $ranToday)) {
