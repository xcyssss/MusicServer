...
function Save-OnlineSearchResultsDb {
    ...
    foreach ($track in $Tracks) {
        $rank++
        $nid = ConvertTo-MusicServerSqlLiteral ([string]$track.identifiers[0].value)
        $base = ConvertTo-MusicServerSqlLiteral ([string]$track.id)
        $alternative = ConvertTo-MusicServerSqlLiteral ('track_netease_' + [string]$track.identifiers[0].value)
        $existing = "SELECT t.id FROM canonical_tracks t, json_each(t.identifiers_json) j WHERE json_extract(j.value,'$.type')='netease' AND CAST(json_extract(j.value,'$.value') AS TEXT)=$nid ORDER BY t.id LIMIT 1"
        $newId = "CASE WHEN EXISTS(SELECT 1 FROM canonical_tracks WHERE id=$base) THEN $alternative ELSE $base END"
        $values = @($track.title,$track.artist,$track.album,$track.duration,'',(ConvertTo-MusicServerJsonArrayText $track.identifiers),(ConvertTo-MusicServerJsonArrayText $track.preview_sources),'[]','','REMOTE',0,$track.created_at,$track.updated_at,1) | ForEach-Object { ConvertTo-MusicServerSqlLiteral $_ }
...
function Invoke-OnlineMusicSearch {
    param([psobject]$Config, [string]$SearchId, [string]$Query)
    ...
    try {
        ...
        Write-MusicServerLog -Path (Join-Path $Config.LogDir 'musicserver-api.log') -Message ("[search] id={0} count={1} error={2} elapsed_ms={3}" -f $SearchId,$count,$code,[int]$clock.Elapsed.TotalMilliseconds)
    }
}
...
function Start-OnlineMusicSearch {
    param([psobject]$Config, [object]$Query)
    ...
    $existing=@(Invoke-MusicServerParamSql -Template "SELECT id FROM online_searches WHERE query=@query AND ((state='RUNNING' AND expires_at>@now) OR (state='DONE' AND created_at>@cache)) ORDER BY created_at DESC LIMIT 1;" -Params @{query=$queryText;now=$now;cache=[DateTime]::UtcNow.AddMinutes(-10).ToString('o')})
    ...
    Invoke-MusicServerParamNonQuery -Template "INSERT INTO online_searches(id,query,state,created_at,expires_at) VALUES(@id,@query,'RUNNING',@now,@expires);" -Params @{id=$id;query=$queryText;now=$now;expires=[DateTime]::UtcNow.AddSeconds(20).ToString('o')} | Out-Null
    $ps=[PowerShell]::Create()
    try {
        [void]$ps.AddScript({
            param($root,$config,$id,$queryText)
            ...
            Invoke-OnlineMusicSearch -Config $config -SearchId $id -Query $queryText
        }).AddArgument($PSScriptRoot).AddArgument($Config).AddArgument($id).AddArgument($queryText)
...
function Get-OnlineMusicSearch {
    ...
    return @{id=$SearchId;query=$search.query;state=$search.state;error=$search.error_code;source='netease';items=$items}
}
