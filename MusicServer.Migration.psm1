...
$script:MigrationConflictCategories = @(
    'SAFE_IDEMPOTENT', 'EXPECTED_LEGACY_OVERLAP', 'STALE_LEGACY',
    'IDENTITY_AMBIGUITY', 'SEMANTIC_CONFLICT', 'MALFORMED'
)

function Add-MigrationSkippedReport {
    param(
        [Parameter(Mandatory)][System.Collections.IDictionary]$Report,
        [Parameter(Mandatory)][ValidateSet('SAFE_IDEMPOTENT','EXPECTED_LEGACY_OVERLAP','STALE_LEGACY','IDENTITY_AMBIGUITY','SEMANTIC_CONFLICT','MALFORMED')][string]$Category,
        [string]$Detail = ''
    )
    $Report['skipped_categories'][$Category] = [int]$Report['skipped_categories'][$Category] + 1
    if ($Detail) { $Report.legacy_ignored += "${Category}: $Detail" }
}
...
function Read-LegacyCollectionSafe {
    ...
    try { return @(Read-StateCollection -Config $Config -Name $Name) }
        ...
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "${Name}.json: $($_.Exception.Message)"
...
function Read-LegacyCsvSafe {
    ...
    try { return @(Import-Csv -LiteralPath $Path -Encoding UTF8 -ErrorAction Stop) }
        ...
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "${Label}: $($_.Exception.Message)"
...
function Read-LegacyEventsSafe {
    ...
    foreach ($line in @(Get-Content -LiteralPath $Path -Encoding UTF8)) {
        ...
        try { $rows += (ConvertFrom-Json -InputObject $line) }
            ...
            Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "events.jsonl line: $($_.Exception.Message)"
            $Report.legacy_ignored += "malformed events.jsonl line: $($_.Exception.Message)"
        }
    }
    return @($rows)
}
...
function Convert-LegacyInt {
    ...
    try { return [int]$Value }
        ...
        if ($Report) { Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "${Label}: $Value" }
...
function Add-MigrationConflictReport {
    param(
        ...
        [ValidateSet('SAFE_IDEMPOTENT','EXPECTED_LEGACY_OVERLAP','STALE_LEGACY','IDENTITY_AMBIGUITY','SEMANTIC_CONFLICT','MALFORMED')][string]$Category = 'EXPECTED_LEGACY_OVERLAP'
    )
    $Report['conflicts'] = [int]$Report['conflicts'] + 1
    $Report['conflict_categories'][$Category] = [int]$Report['conflict_categories'][$Category] + 1
    ...
    if (-not $Report.Contains('conflict_classified_details')) { $Report['conflict_classified_details'] = @() }
    $classifiedDetail = "${Category}:$Detail"
    $classified = @($Report['conflict_classified_details'])
    if ($classified.Count -lt 25 -and $classified -notcontains $classifiedDetail) {
        $Report['conflict_classified_details'] = @($classified + $classifiedDetail)
    }
}
...
function Test-MigrationDailyConflict {
    ...
    $key = "$Date`:$Rank"
    if ($SeenDailyKeys.Contains($key)) { return 'EXPECTED_LEGACY_OVERLAP' }
    $SeenDailyKeys[$key] = $true
    try {
        $rows = @(Invoke-MusicServerParamSql -Template 'SELECT track_id FROM daily_recommendations WHERE date = @date AND rank = @rank LIMIT 1;' -Params @{ date = $Date; rank = $Rank })
        if ($rows.Count -eq 0) { return '' }
        if ($TrackId -and [string]$rows[0].track_id -eq $TrackId) { return 'SAFE_IDEMPOTENT' }
...
function Get-MigrationCanonicalConflictCategory {
    param(
        [Parameter(Mandatory)][string]$TrackId,
        [Parameter(Mandatory)][psobject]$LegacyTrack,
        [bool]$AlreadySeen = $false
    )
    if ($AlreadySeen) { return 'EXPECTED_LEGACY_OVERLAP' }
    try {
        $rows = @(Invoke-MusicServerParamSql -Template 'SELECT id, title, artist, duration, status, local_song_id, identifiers_json, preview_sources_json, download_candidates_json FROM canonical_tracks WHERE id = @id LIMIT 1;' -Params @{ id = $TrackId })
        if ($rows.Count -eq 0) { return '' }
        $row = $rows[0]
        $hasDuration = $null -ne $LegacyTrack.PSObject.Properties['duration'] -or $null -ne $LegacyTrack.PSObject.Properties['Duration']
        $hasIdentifiers = $null -ne $LegacyTrack.PSObject.Properties['identifiers'] -or $null -ne $LegacyTrack.PSObject.Properties['Identifiers']
        $hasPreview = $null -ne $LegacyTrack.PSObject.Properties['preview_sources'] -or $null -ne $LegacyTrack.PSObject.Properties['PreviewSources']
        $hasCandidates = $null -ne $LegacyTrack.PSObject.Properties['download_candidates'] -or $null -ne $LegacyTrack.PSObject.Properties['DownloadCandidates']
        $legacyIdentifiers = if ($hasIdentifiers) { ConvertTo-Json -InputObject @(Get-OptionalProperty $LegacyTrack 'identifiers' (Get-OptionalProperty $LegacyTrack 'Identifiers' @())) -Compress -Depth 10 } else { '' }
        $legacyPreview = if ($hasPreview) { ConvertTo-Json -InputObject @(Get-OptionalProperty $LegacyTrack 'preview_sources' (Get-OptionalProperty $LegacyTrack 'PreviewSources' @())) -Compress -Depth 10 } else { '' }
        $legacyCandidates = if ($hasCandidates) { ConvertTo-Json -InputObject @(Get-OptionalProperty $LegacyTrack 'download_candidates' (Get-OptionalProperty $LegacyTrack 'DownloadCandidates' @())) -Compress -Depth 10 } else { '' }
        $same = ([string]$row.title -eq [string](Get-OptionalProperty $LegacyTrack 'title')) -and
            ([string]$row.artist -eq [string](Get-OptionalProperty $LegacyTrack 'artist')) -and
            (-not $hasDuration -or [int]$row.duration -eq [int](Get-OptionalProperty $LegacyTrack 'duration' (Get-OptionalProperty $LegacyTrack 'Duration' 0))) -and
            ([string]$row.status -eq [string](Get-OptionalProperty $LegacyTrack 'status' 'REMOTE')) -and
            ([string]$row.local_song_id -eq [string](Get-OptionalProperty $LegacyTrack 'local_song_id')) -and
            (-not $hasIdentifiers -or [string]$row.identifiers_json -eq [string]$legacyIdentifiers) -and
            (-not $hasPreview -or [string]$row.preview_sources_json -eq [string]$legacyPreview) -and
            (-not $hasCandidates -or [string]$row.download_candidates_json -eq [string]$legacyCandidates)
        if ($same) { return 'SAFE_IDEMPOTENT' }
        return 'STALE_LEGACY'
    } catch { return 'IDENTITY_AMBIGUITY' }
}
...
function Add-MigrationEventStatement {
    ...
    if (-not $eventType) {
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail 'events.jsonl row: event missing'
...
function Add-MigrationDailyRecommendationStatement {
    ...
    if (-not $title -or -not $date -or -not $trackId) {
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail 'recommendation row (title/date/track_id missing)'
    ...
    if ($rank -le 0) {
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "recommendation rank for $title"
        $Report.legacy_ignored += "malformed recommendation row: invalid rank for $title"
        return $false
    }
    $dailyConflict = ''
    if ($SeenDailyKeys) {
        $dailyConflict = Test-MigrationDailyConflict -Date $date -Rank $rank -TrackId $trackId -SeenDailyKeys $SeenDailyKeys
        if ($dailyConflict) { Add-MigrationConflictReport -Report $Report -Category $dailyConflict -Detail "daily:$date`:$rank" }
    }
    $neteaseId = [string](Get-OptionalProperty $Row 'netease_id' (Get-OptionalProperty $Row 'NeteaseId'))
...
function Add-MigrationRecommendationFileStatement {
    param(
        [Parameter(Mandatory)][AllowEmptyCollection()][System.Collections.Generic.List[string]]$Statements,
        [Parameter(Mandatory)][psobject]$Row,
        [Parameter(Mandatory)][System.Collections.IDictionary]$Report
    )
    $file = [string](Get-OptionalProperty $Row 'File' (Get-OptionalProperty $Row 'file'))
    if (-not $file) {
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail 'recommendation file metadata missing file name'
        return $false
    }
    $title = [string](Get-OptionalProperty $Row 'Title' (Get-OptionalProperty $Row 'title'))
    $artist = [string](Get-OptionalProperty $Row 'Artist' (Get-OptionalProperty $Row 'artist'))
    ...
    if (-not $trackId -and -not $title) {
        Add-MigrationSkippedReport -Report $Report -Category 'MALFORMED' -Detail "recommendation file metadata missing identity: $file"
        return $false
    }
    $date = [string](Get-OptionalProperty $Row 'Date' (Get-OptionalProperty $Row 'date'))
    $neteaseId = [string](Get-OptionalProperty $Row 'NeteaseId' (Get-OptionalProperty $Row 'netease_id'))
    $album = [string](Get-OptionalProperty $Row 'Album' (Get-OptionalProperty $Row 'album'))
    $duration = Convert-LegacyInt -Value (Get-OptionalProperty $Row 'Duration' (Get-OptionalProperty $Row 'duration' 0)) -Report $Report -Label "recommendation file duration for $file"
    $seed = [string](Get-OptionalProperty $Row 'FromSeed' (Get-OptionalProperty $Row 'seed_source'))
    $now = Get-NowIso
    $p = @{
        file = ConvertTo-MusicServerSqlLiteral $file; tid = ConvertTo-MusicServerSqlLiteral $trackId
        date = ConvertTo-MusicServerSqlLiteral $date; nid = ConvertTo-MusicServerSqlLiteral $neteaseId
        title = ConvertTo-MusicServerSqlLiteral $title; artist = ConvertTo-MusicServerSqlLiteral $artist
        album = ConvertTo-MusicServerSqlLiteral $album; duration = ConvertTo-MusicServerSqlLiteral $duration
        seed = ConvertTo-MusicServerSqlLiteral $seed; now = ConvertTo-MusicServerSqlLiteral $now
    }
    [void]$Statements.Add("INSERT OR IGNORE INTO recommendation_files (file_name,track_id,date,netease_id,title,artist,album,duration,seed_source,created_at,updated_at) VALUES ($($p.file),$($p.tid),$($p.date),$($p.nid),$($p.title),$($p.artist),$($p.album),$($p.duration),$($p.seed),$($p.now),$($p.now))")
    return $true
}
...
function Invoke-MusicServerMigration {
    ...
    $conflictCategories = [ordered]@{}
    $skippedCategories = [ordered]@{}
    foreach ($category in $script:MigrationConflictCategories) {
        $conflictCategories[$category] = 0
        $skippedCategories[$category] = 0
    }
    $report = [ordered]@{
        status = 'PENDING'; source_counts = @{}; database_counts = @{}; imported = @{}; skipped = 0
        skipped_categories = $skippedCategories; legacy_ignored = @(); conflicts = 0
        conflict_categories = $conflictCategories; conflict_details = @(); backup_path = ''; idempotent = $false
    }
    ...
    $report.imported = @{ tracks = 0; recommendations = 0; history = 0; history_files = 0; wanted = 0; providers = 0; events = 0; accepted = 0; rejected = 0; lyrics_fallback = 0; display = 0; explicit_likes = 0 }
        ...
        foreach ($row in $historyCsvRows) {
            if (Add-MigrationRecommendationFileStatement -Statements $previewStatements -Row $row -Report $report) { $report.imported.history_files++ }
        }
        foreach ($row in $acceptedRows) {
            ...
            if (-not $title -and -not $neteaseId) { Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'accepted.csv row: title and NeteaseId missing'; continue }
            $report.imported.accepted++
        }
        foreach ($row in $rejectedRows) {
            $title = [string](Get-OptionalProperty $row 'Title'); $neteaseId = [string](Get-OptionalProperty $row 'NeteaseId')
            if (-not $title -and -not $neteaseId) { Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'rejected.csv row: title and NeteaseId missing'; continue }
        ...
        foreach ($row in $lyricsRows) {
            ...
            if ($matched -and $status -in @('OK','NO_LYRIC')) { $report.imported.lyrics_fallback++ } else { $invalidLyrics++; Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'lyrics_report row' }
        }
        ...
        foreach ($track in $jsonTracks) {
            ...
            if (-not $trackId) { Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'tracks.json row: id missing'; continue }
            $alreadySeenTrack = $seenTrackIds.ContainsKey($trackId)
            $trackConflict = $alreadySeenTrack
            ...
            if ($trackConflict) {
                $category = Get-MigrationCanonicalConflictCategory -TrackId $trackId -LegacyTrack $track -AlreadySeen $alreadySeenTrack
                if (-not $category) { $category = 'IDENTITY_AMBIGUITY' }
                $report.skipped++
                Add-MigrationSkippedReport -Report $report -Category $category -Detail "canonical:$trackId"
                Add-MigrationConflictReport -Report $report -Category $category -Detail "canonical:$trackId"
            } else { $report.imported.tracks++ }
        }
        $seenWantedIds = @{}
        foreach ($wanted in $jsonWanted) {
            $wantedTrackId = [string](Get-OptionalProperty $wanted 'track_id')
            if (-not $wantedTrackId) { Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'wanted.json row: track_id missing'; continue }
            ...
            if ($wantedConflict) {
                $report.skipped++
                Add-MigrationSkippedReport -Report $report -Category 'STALE_LEGACY' -Detail "wanted:$wantedTrackId"
                Add-MigrationConflictReport -Report $report -Category 'STALE_LEGACY' -Detail "wanted:$wantedTrackId"
            } else { $report.imported.wanted++ }
        }
        $seenProviderNames = @{}
        foreach ($prov in $jsonProviders) {
            $providerName = [string](Get-OptionalProperty $prov 'provider')
            if ($providerName -eq 'bilibili') {
                $report.legacy_ignored += 'legacy provider: bilibili (ignored)'
                $report.skipped++
                Add-MigrationSkippedReport -Report $report -Category 'STALE_LEGACY' -Detail 'legacy provider: bilibili ignored'
                Add-MigrationConflictReport -Report $report -Category 'STALE_LEGACY' -Detail 'provider:bilibili (ignored)'
                continue
            }
            if ($providerName -in @('local','bilibili_search','bilibili_download')) {
                ...
                if ($providerConflict) {
                    $report.skipped++
                    Add-MigrationSkippedReport -Report $report -Category 'STALE_LEGACY' -Detail "provider:$providerName"
                    Add-MigrationConflictReport -Report $report -Category 'STALE_LEGACY' -Detail "provider:$providerName"
                } else { $report.imported.providers++ }
            } else { Add-MigrationSkippedReport -Report $report -Category 'IDENTITY_AMBIGUITY' -Detail "unknown provider: $providerName" }
        }
    ...
    $importedAccepted = 0; $importedRejected = 0; $importedHistoryFiles = 0; $importedLyrics = 0; $importedEvents = 0; $displayCount = 0; $skipped = 0; $conflicts = 0
        $report.imported = @{ tracks = 0; recommendations = 0; history = 0; history_files = 0; wanted = 0; providers = 0; events = 0; accepted = 0; rejected = 0; lyrics_fallback = 0; display = 0; explicit_likes = 0 }
    ...
    foreach ($row in $historyCsvRows) {
        if (Add-MigrationRecommendationFileStatement -Statements $statements -Row $row -Report $report) { $importedHistoryFiles++ }
    }
    ...
    foreach ($row in $acceptedRows) {
        ...
        if ($file) { if (Add-MigrationRecommendationFileStatement -Statements $statements -Row $row -Report $report) { $importedHistoryFiles++ } }
        $importedAccepted++
    }
    foreach ($row in $rejectedRows) {
        ...
        if (Get-OptionalProperty $row 'File') { if (Add-MigrationRecommendationFileStatement -Statements $statements -Row $row -Report $report) { $importedHistoryFiles++ } }
        $importedRejected++
    }
    foreach ($row in $lyricsRows) {
        ...
        if (-not $matched -or $status -notin @('OK','NO_LYRIC')) {
            Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'lyrics_report row'
            continue
        }
    ...
    foreach ($w in $jsonWanted) {
        ...
        if (-not $trackId) { Add-MigrationSkippedReport -Report $report -Category 'MALFORMED' -Detail 'wanted.json row: track_id missing'; continue }
        ...
        if ($wantedConflict) {
            $skipped++
            Add-MigrationSkippedReport -Report $report -Category 'STALE_LEGACY' -Detail "wanted:$trackId"
            Add-MigrationConflictReport -Report $report -Category 'STALE_LEGACY' -Detail "wanted:$trackId"
            continue
        }
    ...
    foreach ($prov in $jsonProviders) {
        ...
        if ($pname -notin $validProviders) {
            $report.legacy_ignored += "unknown provider: $pname"
            Add-MigrationSkippedReport -Report $report -Category 'IDENTITY_AMBIGUITY' -Detail "unknown provider: $pname"
            continue
        }
        ...
        if ($providerConflict) {
            $skipped++
            Add-MigrationSkippedReport -Report $report -Category 'STALE_LEGACY' -Detail "provider:$pname"
            Add-MigrationConflictReport -Report $report -Category 'STALE_LEGACY' -Detail "provider:$pname"
            continue
        }
    ...
    try {
        ...
        $report.imported = @{ tracks = $importedTracks; recommendations = $importedRecs; history = $importedHistory; history_files = $importedHistoryFiles; wanted = $importedWanted; providers = $importedProviders; events = $importedEvents; accepted = $importedAccepted; rejected = $importedRejected; lyrics_fallback = $importedLyrics; display = $displayCount; explicit_likes = [int]$report.imported.explicit_likes }
